...
export class MonchBuffer {
  ...
  constructor(...slices: Iterable<number>[]) {
    ...
    switch (slices.length) {
      ...
      default:
        ...
        this.typedArray = Uint8Array.from(slices.reduce<number[]>((p, c) => {
          p.push(...c)
          return p
        }, []))
        break
  ...
  readUint16BE(offset: number, count = 1) {
    ...
    for (let i = 0; i < count; i++) {
      temp[i] = this.typedArray[offset + (1 + (i * 2))]
  ...
  readUint16LE(offset: number, count = 1) {
    ...
    const temp = new Uint16Array(count)
    for (let i = 0; i < count; i++) {
      temp[i] = this.typedArray[offset + (i * 2)]
  ...
  readUint32BE(offset: number, count = 1) {
    ...
    for (let i = 0; i < count; i++) {
      temp[i] = this.typedArray[offset + (3 + (i * 4))]
  ...
  readUint32LE(offset: number, count = 1) {
    ...
    const temp = new Uint32Array(count)
    for (let i = 0; i < count; i++) {
      temp[i] = this.typedArray[offset + (i * 4)]
  ...
  readUint64BE(offset: number, count = 1) {
    ...
    for (let i = 0; i < count; i++) {
      ...
      temp[i] = BigInt(this.typedArray[offset + (7 + (i * 8))])
  ...
  readUint64LE(offset: number, count = 1) {
    ...
    const temp = new BigUint64Array(count)
    for (let i = 0; i < count; i++) {
      temp[i] = BigInt(this.typedArray[offset + (i * 8)])
